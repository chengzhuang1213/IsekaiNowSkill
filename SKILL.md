...
8. Output a complete post package in Chinese unless the user asks for another language:
   ...
   - image prompt
   - likes line
   - 6-10 comments or interactions for full finished images, including ordinary comments, occasional poster replies, and occasional second-level replies when space allows
   - optional full finished-image plan
9. Keep the post short, social, and strongly character-voiced. The comments should feel like the specific characters talking, not interchangeable fantasy NPCs: preserve each character's likely rhythm, confidence level, humor, worries, teasing style, formality, and relationship to the poster.
...
- Treat `assets/Samples/sample-airship-port-native-1206x2622.png` as the approved baseline sample for finished-image layout: native long screen, no legacy upscale padding, large main illustration, and expanded lower comment area. Do not copy its bottom input controls.
...
- The whole image should be the status. Do not place a large white social card inside an unrelated poster background. The feed frame, paper/stone/map texture, ornaments, text, main illustration, likes, and comments must feel like one designed interface.
...
- Hair length is a hard identity anchor, not a styling detail. Never change a character from short hair to long hair, long hair to short hair, loose hair to tied hair, or tied hair to loose hair unless the user explicitly asks for that transformation. When multiple named characters appear together, write separate hair-length locks for each one before generating the main image.
...
- Real UI text must be rendered locally whenever possible. Do not depend on image generation for readable captions, likes, or comments. Keep in-image generated text short and story-relevant only.
...
- Comments and replies are part of the character writing, not filler. Make every line plausible for that named character: shy characters hesitate, dramatic characters dramatize, analytical characters notice details, ambitious characters promote themselves, and close friends tease in ways that match their relationship.
- For full finished social-feed images, prefer a lively lower section over empty space: use ordinary comments plus 1-3 poster replies and 1-3 second-level replies when the canvas has room. Do not add replies just to inflate length; each reply should advance the joke, relationship, or small story hook.
- Ordinary comments use the standard left avatar plus right bubble layout. Poster replies should remain left-to-right readable, but may place the poster avatar on the right side with the reply bubble on the left, so the host's participation is easy to distinguish at a glance.
- Label second-level interactions clearly in the comment name, such as `千砂都 回复 雫：...`, `露比 回复 四季：...`, or another short given-name form. Keep these labels compact so the reply still reads like a social comment rather than a dialogue transcript.
- Never generate the bottom comment input field or its adjacent action icons in finished social-feed images. Omit controls such as `写评论...`, heart/like icons, share arrows, send buttons, or other bottom interaction icons. Let the comment area and themed frame end cleanly near the bottom safe area instead.
...
**本地合成要求**：
固定尺寸 1206x2622，竖屏，比例 19.5:9；根据地点和正文选择异世界主题皮肤；整张图就是朋友圈状态而不是白框套图；头像全部从 `assets/Avatars` 圆裁；正文、点赞和评论用本地字体渲染；不要生成底部评论输入框、心形图标、分享箭头或其他底部操作图标；主图、点赞和评论区都不能互相遮挡。
```
